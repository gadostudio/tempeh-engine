...
namespace Tempeh::Event
{
	...
	{
		if (event.type != Type::MouseMove) LOG_INFO("{}", magic_enum::enum_name(event.type));
	...
	{
		LOG_INFO("Mouse - Key {}({}) {}", magic_enum::enum_name(key_code), key_code, key_state == KeyState::Pressed ? "Pressed" : "Released");
		switch (key_code)
		{
		case Left:
