                                ...
                                match arg {
                                    GenericArgument::Type(ty) => match ty {
                                        Type::Reference(ty) => {
                                            ...
                                            if mutable {
                                                query.push(
                                                    parse_quote!(::tempeh_ecs::TryWrite<#elem>),
                                                );
                                            } else {
                                                query.push(
                                                    parse_quote!(::tempeh_ecs::TryRead<#elem>),
                                                );
                                            }
                                        }
                                        _ => {
                    ...
                    Type::Reference(ty) => {
                        ...
                        match attribute {
                            ...
                            None => {
                                ...
                                let elem = &ty.elem;
                                if mutable {
                                    query.push(parse_quote!(::tempeh_ecs::Write<#elem>));
                                } else {
                                    query.push(parse_quote!(::tempeh_ecs::Read<#elem>));
                                }
                            }
                        }
                    }
...
impl Config {
    ...
    fn generate(&mut self) -> Result<proc_macro2::TokenStream, Error> {
        ...
        let result = quote! {
            #visibility fn #constructor_name<#generic_params>(#(#fn_params),*) -> impl ::tempeh_ecs::systems::Runnable
            #where_clause
            {
                #builder
            }
